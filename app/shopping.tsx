...
import { shoppingOutlets, ShoppingOutlet } from '../mock/shopping';

export default function ShoppingPage() {
  const router = useRouter();
  const [filter, setFilter] = useState<string>('All');

  const categories = ['All', 'Duty Free', 'Beauty & Fashion', 'Electronics', 'Books & Convenience', 'Apparel', 'Gifts'];

  const filteredOutlets = filter === 'All' 
    ? shoppingOutlets 
    : shoppingOutlets.filter(outlet => outlet.category === filter);

  const renderShoppingItem = ({ item }: { item: ShoppingOutlet }) => (
